...
$allNamed = Get-Process -Name $ProcessName -ErrorAction SilentlyContinue
# Toolkit only connects to Antigravity IDE's process; others (Windsurf, Codeium) do not count
$antigravityProcesses = @()
if ($allNamed) {
    foreach ($p in $allNamed) {
        try {
            $path = $p.Path
            if ($path -and ($path -match "antigravity|Antigravity")) { $antigravityProcesses += $p }
        } catch { }
    }
}

if ($antigravityProcesses -and $antigravityProcesses.Count -gt 0) {
    Write-Host "FOUND: $($antigravityProcesses.Count) Antigravity $FullExe process(es)." -ForegroundColor Green
    $antigravityProcesses | Format-Table Id, Name, CPU, WorkingSet -AutoSize
    Write-Host ""
    Write-Host "Status: CONNECTED - the Antigravity Toolkit should be able to reach the language server." -ForegroundColor Green
    exit 0
} else {
    Write-Host "NOT FOUND: no Antigravity $FullExe process (path containing 'antigravity')." -ForegroundColor Yellow
    Write-Host ""
    if ($allNamed) {
        Write-Host "Other $FullExe processes on this machine (Toolkit expects Antigravity IDE, not these):" -ForegroundColor Cyan
        $allNamed | Select-Object Id, ProcessName, @{N='Path';E={$_.Path}} | Format-Table -AutoSize -Wrap
        Write-Host ""
    }
